...
! function (t) {
		...
		var r = e(3),
			...
			u = function () {
				...
				return n.prototype.ResetTeethState = function () {
					...
					r.select("svg").attr("preserveAspectRatio", "xMinYMin meet").attr("viewBox", "0 0 1050 800"),
						r.selectAll(".implante").style("fill", "transparent"),
						r.selectAll(".corona").style("stroke", "#transparent"), r.selectAll(".corona").style("fill", "#transparent"),
						r.selectAll(".raiz").style("stroke", "#282828"), r.selectAll(".raiz").style("fill", "#ECDC9E"),
						r.selectAll(".nervio").style("stroke", "transparent"), r.selectAll(".nervio").style("fill", "transparent"),
						r.selectAll(".dentina").style("stroke", "transparent"), r.selectAll(".dentina").style("fill", "transparent"),
						r.selectAll(".cuello").style("stroke", "unset"), r.selectAll(".cuello").style("fill", "transparent"),
						r.selectAll(".extraccion").style("stroke", "transparent"), r.selectAll(".extraccion").style("fill", "transparent"),
						r.selectAll(".puente").style("stroke", "transparent"), r.selectAll(".puente").style("fill", "transparent"),
						r.selectAll(".protesis").style("stroke", "transparent"), r.selectAll(".protesis").style("fill", "transparent"),
						r.selectAll(".puenteVarios").style("stroke", "transparent"), r.selectAll(".puenteVarios").style("fill", "transparent")
					...
					t.forEach(function (t) {
						...
						var n = this;
						this.Paths.on("mouseenter", function (e, i) {
							var u = r.select("#" + t("path")[i].id);
							if (u.attr("class")) {
								var a = u.attr("class").split(" ")[0],
									c = u.attr("class").split(" ")[1];
								if (n.Selected = u.attr("class"), o.isPeriodonto(c)) {
									var s = "." + a;
									if (n.IsImplante(a)) {
										
										var f = r.selectAll(s + ".implante");
										//f.style("fill", "#537EDE")
									} else {
										var l = r.selectAll(s + ".raiz");
										//l.style("stroke", "#537EDE")
									}
									n.PathId = t("path")[i].id
								}
								if (o.isOdonto(c)) {
									var s = "." + a,
										l = r.selectAll(s + "." + c);
									l.style("stroke", "#537EDE"), n.PathId = t("path")[i].id
								}
							}
						})
					}, n.prototype.MouseLeave = function () {
						var n = this;
						this.Paths.on("mouseleave", function (e, i) {
							var u = r.select("#" + t("path")[i].id);
							if (u.attr("class")) {
								var a = u.attr("class").split(" ")[0],
									c = u.attr("class").split(" ")[1],
								...
								if (n.IsImplante(a)) {
									...
									try {
										var h = r.selectAll(s + ".raiz"),
											p = n.GetCurrentState(a),
											d = n.GetColorOfState(p.state);
										h.style("stroke", d)
									} catch (e) {
								...
								if (o.isOdonto(c)) {
									var v = r.selectAll(s + "." + c);
									v.style("stroke", "#292929")
								}
								n.PathId = ""
							}
						})
				...
				}, n.prototype.IsImplante = function (t) {
					var n = this.GetCurrentState(t);
					try {
						return "implante" == n.state
					} catch (e) {
	...
	function r(t) {
		return e(["raiz", "corona", "dentina", "nervio", "cuello", "implante", "ausente", "sellante", "extraccion", "puente", "protesis", "puenteVarios"], [t])
	}
